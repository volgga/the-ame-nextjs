...
- **Вебхук** `POST /api/payments/tinkoff/notification`: проверка подписи, обновление статуса заказа (paid / failed / canceled).
- **Страницы результата**:
...
2. Открыть корзину (страница `/cart` или иконка корзины в шапке).
...
Для проверки вебхука локально: поднять туннель (ngrok), подставить `TINKOFF_NOTIFICATION_URL=https://xxx.ngrok.io/api/payments/tinkoff/notification` в `.env.local`.
