...
PM2_APP_NAME="${2:-${PM2_APP_NAME:-theame-next}}"

echo "Deploy path: $DEPLOY_PATH | PM2: $PM2_APP_NAME"
cd "$DEPLOY_PATH" || { echo "Error: $DEPLOY_PATH not found"; exit 1; }

# Очистка старой сборки перед пересборкой (устраняет 404 на чанках после деплоя)
rm -rf .next
...
# Standalone: явное копирование static и public (Next.js не всегда копирует автоматически)
if [ -d ".next/standalone" ]; then
  echo "Copying static and public into standalone..."
