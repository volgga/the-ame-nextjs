...
```bash
cd $DEPLOY_PATH && npm ci && NODE_OPTIONS="--max-old-space-size=1536" npm run build && pm2 start ecosystem.config.cjs && pm2 save && pm2 startup
```

> **Серверы с 2ГБ RAM:** используйте `NODE_OPTIONS="--max-old-space-size=1536"` перед `npm run build`, чтобы избежать зависания на этапе TypeScript. Next.js сконфигурирован с `ignoreBuildErrors` для билда.
...
```bash
cd $DEPLOY_PATH && npm ci && NODE_OPTIONS="--max-old-space-size=1536" npm run build && pm2 restart ecosystem.config.cjs
```
