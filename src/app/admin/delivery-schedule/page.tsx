  ...
  async function handleAddDay() {
    ...
    try {
      ...
      if (!result.ok || !result.data) {
        const apiError = result.data && typeof (result.data as any).error === "string" ? (result.data as any).error : null;
        const message = apiError
          ? `${apiError}${result.message ? ` (${result.message})` : ""}`
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-[#111]">Время доставки</h2>
        <button
          type="button"
          onClick={() => {
            setAddingDay(true);
      ...
      {addingDay && (
        <div className="rounded-xl border border-border-block bg-white p-4">
          <h3 className="mb-3 font-medium text-[#111]">Добавить день</h3>
              ...
              <div key={day.id} className="rounded-xl border border-border-block bg-white overflow-hidden">
                <div className="flex items-center justify-between p-4">
                  <div className="flex-1">
                    <h3 className="font-medium text-[#111]">{formatDate(day.date)}</h3>
