...
function validateAndParse(body: unknown): GiftHintFormData | { error: string } {
  ...
  const result: GiftHintFormData = { phone: phoneValidation.normalized! };
  ...
  const recipientNameValidation = validateStringField(data.recipientName, "recipientName", 80, false);
  if (!recipientNameValidation.valid) {
    return { error: recipientNameValidation.error! };
  }
  result.recipientName = recipientNameValidation.normalized;

  const preferredDateValidation = validateStringField(data.preferredDate, "preferredDate", 50, false);
  if (!preferredDateValidation.valid) {
    return { error: preferredDateValidation.error! };
  }
  result.preferredDate = preferredDateValidation.normalized;
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      const payload = createGiftHintPayload(parsed);
    ...
    try {
      const message = formatGiftHintMessage(parsed, leadId);
