/**
 * POST /api/forms/preorder — обработка формы "Предзаказ".
 * Валидирует данные, сохраняет лид в БД и отправляет уведомление в Telegram.
 */

import { NextResponse } from "next/server";
import { sendToTelegram } from "@/lib/telegram";
import { formatPreorderMessage, type PreorderFormData } from "@/lib/format";
...
import { createPreorderPayload } from "@/lib/leads";
import { logLeadEvent, getIPFromRequest, getUserAgentFromRequest } from "@/lib/leadEvents";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const FORM_TYPE = "preorder";

// Периодическая очистка rate limit (каждый 10-й запрос)
let requestCount = 0;

/**
 * Валидирует и нормализует данные формы "Предзаказ".
 */
function validateAndParse(body: unknown): PreorderFormData | { error: string } {
  ...
  // Валидация даты предзаказа (обязательное поле)
  const preorderDateValidation = validateStringField(data.preorderDate, "preorderDate", 40, true);
  if (!preorderDateValidation.valid || !preorderDateValidation.normalized) {
    return { error: preorderDateValidation.error ?? "Поле 'preorderDate' обязательно" };
  }

  const result: PreorderFormData = {
    phone: phoneValidation.normalized!,
    preorderDate: preorderDateValidation.normalized,
  };
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      const payload = createPreorderPayload(parsed);
    ...
    try {
      const message = formatPreorderMessage(parsed, leadId);
