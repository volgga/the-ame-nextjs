  ...
  try {
    ...
    if (existing?.id) {
      ...
      const { data, error } = await (supabase as any)
        ...
        .insert({
          rating_count: 50,
          review2_text: "Прекрасная мастерская цветов. Заказываю букеты не первый раз. Все очень стильно, красиво, качественно. Радует глаз и согревает душу. Все, кому я дарю букеты от Flowerna, в восторге! Однозначная рекомендация. Помимо качества и стиля, всегда все четко и вовремя. Что тоже очень и очень важно. Спасибо за то, что дарите красоту и хорошее настроение",
          review3_text: "Всем сердцем люблю Flowerna ❤️ Цветочный с особенной, теплой атмосферой 😊 Букеты как произведение искусства, каждый создан с душой и тонким чувством прекрасного 😊 Сервис Flowerna – это высший уровень, такого дружелюбного и молниеносного взаимодействия с клиентом я ранее не встречала 😊 Flowerna, Вы просто разрыв сердца ❤️ Желаю процветания такому крутому бизнесу!!! ❤️",
          faq_items: parsed.data.items,
          updated_at: new Date().toISOString(),
        })
        .select("faq_items")
