...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error("[admin/blog/upload] Ошибка парсинга FormData:", e);
      return NextResponse.json({ error: "Ошибка чтения данных формы" }, { status: 400 });
    }
    const file = formData.get("file") as File | null;
    const postId = formData.get("postId") as string | null; // опционально, для организации файлов
    ...
    // Путь: posts/${postId}/cover-${timestamp}.${ext} для обложки
    const storagePath = postId ? `posts/${postId}/cover-${timestamp}.${ext}` : `temp/cover-${timestamp}.${ext}`;
    ...
    if (error) {
      console.error("[admin/blog/upload] Upload error:", JSON.stringify(error, null, 2));
    ...
    console.error("[admin/blog/upload POST]", e);
