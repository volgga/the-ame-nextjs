  ...
  return (
    ...
    >
      ...
      >
        <button
          type="button"
          aria-label={page.is_active ? "Скрыть" : "Показать"}
          disabled={isToggling}
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
        ...
        >
          {page.is_active ? <Eye size={iconSize} strokeWidth={2} /> : <EyeOff size={iconSize} strokeWidth={2} />}
        </button>
        <button
          type="button"
      ...
      <div className="flex flex-1 flex-col justify-center pt-6">
        <p className="font-medium text-[#111] text-lg leading-tight">{page.title}</p>
        <p className="mt-1 text-sm text-gray-500">/{page.slug}</p>
        <p className={`mt-1 text-sm ${page.is_active ? "text-color-text-main" : "text-gray-400"}`}>
