    ...
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: Z_OVERLAY }}>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-border-block">
          <h2 className="text-xl font-semibold text-color-text-main">{success ? "Предзаказ принят" : "Предзаказ"}</h2>
          <button
            type="button"
            onClick={onClose}
          ...
          {success ? (
            <div className="text-center py-8">
              ...
              <p className="text-color-text-secondary text-sm mb-6">
                Заявка на предзаказ принята. Мы свяжемся с вами в течение 5 минут.
              </p>
              <button
                type="button"
                onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-4 min-w-0 max-w-full">
            ...
            <div className="w-full min-w-0">
              ...
              <input
                ...
                min={getTodayISO()}
                lang="ru"
                className="w-full min-w-0 box-border appearance-none bg-white px-4 h-12 border border-border-block rounded-lg text-color-text-main focus:outline-none focus:ring-2 focus:ring-[rgba(111,131,99,0.5)] focus:border-border-block overflow-hidden"
              />
